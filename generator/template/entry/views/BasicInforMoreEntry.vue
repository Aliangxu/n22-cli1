  ...
  methods: {
    ...
    */
    checkData(data) {
      ...
      if (data.contactAddressCode.indexOf(",") == -1) {
        this.alert("联系地址为必填项，请进行填写");
        return false;
      }
      if (data.contactAddressCode.indexOf("null") > -1) {
        this.alert("联系地址为必填项，请进行填写");
        return false;
      }
      this.personalInformation.contactAddress = this.getAddressDesc(
        data.contactAddressCode.split(",")[0],
        data.contactAddressCode.split(",")[1],
      );
      //详细地址
      ...
      if (this.uid != undefined && this.uid !== "") {
        this.requestQueryUserInfo(
          ...
          body => {
            ...
            this.$nextTick(() => {
              let province = body.data.contactAddressProvince == null ? "" : body.data.contactAddressProvince;
              let city = body.data.contactAddressCity == null ? "" : body.data.contactAddressCity;
              let address = null;
              if (province != "" && city != "") {
              ...
              this.personalInformation = {
                ...
                contactAddressCode: address == null ? "" : address,
