<template>
  <div class="proposalBook-data-entry">
    ...
    >
      ...
      <n22-field class="doc-info" :class="['bottom-height-all']">
        <div class="doc-arrea">
    ...
    <n22-popup v-model="isPopupShowProEntry" position="bottom" @maskClick="riskMaskClick" class="pro-popup">
      ...
      <div class="pro-select-popup-content n22-popup-content" v-if="isPopupShowProEntry">
        <div v-for="(master, mi) in masterProData" :key="`master${mi}`">
          <n22-field :title="master.name" @headerClick="selectMasterPro(master, mi)">
            <div class="action-container" slot="action">
              <n22-icon
                :style="{ color: mi === 0 || master.disabled ? '#f1eff0' : '' }"
            ...
            <transition name="n22-base-fade">
              <div class="pro-popup-cont" v-if="master.isCheck">
                ...
                <div v-if="master.xjdraw" class="pro-apend draw">
                  <n22-cell-item title="减保领取现金价值">
                    <template slot="right">
                      <n22-icon name="add_pro" @click="proAppendOrDrawAdd(master.draw)"></n22-icon>
                    </template>
                  </n22-cell-item>
...
export default {
  ...
  data() {
    return {
      ...
      //主险录入信息及附加险录入信息合并集合-临时记录刚刚弹出框时候的险种信息--用于解决更改参数但是没有点击确定而是点击取消的情况
      masterProShowDataLs: [],
      allPremium: 0,
      //保费试算loading
  ...
  methods: {
    ...
    riskMaskClick() {
      /**
       * 修复当选择修改险种弹出后，更改了相关产品的某些字段，但是因为最终没有点击确定，导致再次点击修改弹出险种时候，
       * 绑定的险种对象masterProShowData需要取上一次临时存储的数据，防止在没有查询回显接口的时候出现的回显的问题
       */
      this.masterProShowData = _cloneDeep(this.masterProShowDataLs);
      //蒙层点击取消
    ...
    getProductEntryFun(code, i, type) {
      ...
      getProductEntry({ data: params, config: axiosParams, method: "get" }).then(
        data => {
          ...
          if (this.masterProData && this.masterProData[0]) {
            ...
            //绑定险种，默认将其选中
            let bindProductCode = this.masterProData[0].bind;
            if (bindProductCode && bindProductCode.length > 0) {
              for (let proi = 0; proi < this.masterProData.length; proi++) {
                const pro = this.masterProData[proi];
                if (bindProductCode.indexOf(pro.code) > -1) {
                  this.masterProData[proi].isCheck = true;
                  this.masterProData[proi].disabled = true;
                }
              }
            }
          }
          if (type === 1) {
            ...
            this.masterProShowDataLs = _cloneDeep(this.masterProShowData);
            this.mainSelect = i;
            //录入对象塞入回显值
    ...
    calculateFun: _throttle(function(holder, insured, productList, isLoadding, ls) {
      ...
      const ruleData = productCheck(productList, this.insured.age);
      if (ruleData) {
        this.$toast({
          content: `${ruleData}！`,
          position: "bottom",
    ...
    selectMasterPro(item, i) {
      console.log("%c >>>>>>>>>>>>>>>>>>>>> 险种选择点击", "color:green;", "");
      //主险和默认无法选择的直接跳出
