...
export const productCheck = function(params, age) {
  //遍历主险组合数组
  for (let mpi = 0; mpi < params.length; mpi++) {
    ...
    for (let mpi = 0; mpi < mpro.product.length; mpi++) {
      ...
      if (pro.inputModel === 1 || pro.quantity != undefined) {
        //份数
        if (pro.quantity !== 0 && !pro.quantity) {
      ...
      if (pro.append && pro.append.length > 0) {
        for (let i = 0; i < pro.append.length; i++) {
          ...
          if (Number(item.startYear) > Number(item.endYear)) {
      ...
      if (pro.isDraw && pro.draw && pro.draw.length > 0) {
      ...
      if (pro.xjdraw && pro.draw.length > 0) {
        for (let i = 0; i < pro.draw.length; i++) {
          const item = pro.draw[i];
          if (!item.startYear) {
            return `请输入${showProductName(pro)}被保人领取起始年龄`;
          }
          if (parseInt(item.startYear) < parseInt(age)) {
            return `${showProductName(pro)}被保人领取起始年龄不可低于投保时的年龄`;
          }
          if (!item.endYear) {
            return `请输入${showProductName(pro)}被保人领取结束年龄`;
          }
          if (parseInt(item.endYear) > 106) {
            return `${showProductName(pro)}被保人领取结束年龄不能超过106岁`;
          }
          if (!item.money) {
            return `请输入${showProductName(pro)}每年减额领取金额`;
          }
        }
      }
    }
