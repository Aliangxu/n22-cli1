      ...
      <div class="my-examine-content">
        ...
        <div v-if="isNoConsidereceiptShow">
          ...
          <div
            class="my-examine-content-detail"
            :class="index > 0 ? 'margin-top' : ''"
            v-for="(item, index) in noReceipts"
            :key="index"
          >
            <div class="detail-title">
              <div class="detail-title-icon"></div>
...
export default {
  ...
  methods: {
    ...
    getUserMaintain(listType) {
      ...
      if (this.rankList.length && this.tabs[this.curIndex].flag == "09") {
        this.rankList = [];
      }
      getUserMaintain({ data: request, config: axiosParams, method: "get", isParams: false }).then(
        data => {
          data.data.forEach(item => {
            if (this.tabs[this.curIndex].flag != "09") {
            ...
            if (this.tabs.length && this.tabs[this.curIndex].flag == "09") {
              ...
              this.haveReceipts = this.resData[this.rankList[0].value].haveReceipts;
              this.noReceipts = this.resData[this.rankList[0].value].noReceipts;
            }
          });
        },
        error => {
